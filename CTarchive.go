...
var appName = "CTarchive"
var appVer = "1.0"
var appAuthor = "Michael Gresham"
var appAuthorEmail = "michael.gresham@hpe.com"
var pgmDescription = fmt.Sprintf("%s: Move serials to archive in Central.",appName)
...
func archiveSerial(central_info goCentral.Central_struct, serial string) int {

  access_token := central_info.Token
  base_url := central_info.Base_url
  api_function_url := fmt.Sprintf("%s/platform/device_inventory/v1/devices/archive",base_url)
...
//======================================================
  ...
  for _, line := range fileLines {
     ...
     respCode := archiveSerial(central_info,line)
     if (respCode != 200) {
        fmt.Printf("\nERROR moving %s to archive. Response code: %d\n",line,respCode)
     } else {
        fmt.Printf("\nSuccessfully moved %s to archive. Response code: %d\n",line,respCode)
