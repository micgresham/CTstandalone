...
var appName = "CTunarchive"
var appVer = "1.0"
var appAuthor = "Michael Gresham"
var appAuthorEmail = "michael.gresham@hpe.com"
var pgmDescription = fmt.Sprintf("%s: Move serials from the archive to the live inventory in Central.",appName)
...
func unarchiveSerial(central_info goCentral.Central_struct, serial string) int {

  access_token := central_info.Token
  base_url := central_info.Base_url
  api_function_url := fmt.Sprintf("%s/platform/device_inventory/v1/devices/unarchive",base_url)

  jsonPrep := fmt.Sprintf("{\"serials\":[\"%s\"]}",serial)
...
//----------------------------------------
// There is no pre check for this one.
//----------------------------------------
...
//======================================================
  ...
  for _, line := range fileLines {
     ...
     respCode := unarchiveSerial(central_info,line)
     if (respCode != 200) {
        fmt.Printf("\nERROR moving %s to archive. Response code: %d\n",line,respCode)
     } else {
        fmt.Printf("\nSuccessfully moved %s from archive to live inventory. Response code: %d\n",line,respCode)
...
//----------------------------------------
// There is no post check for this one.
//----------------------------------------
