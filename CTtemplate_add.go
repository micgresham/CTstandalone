...
func setFirmware(central_info goCentral.Central_struct, gname string, Tgroup templateGroup) string {
  ...
  if (Tgroup.devtype == "MobilityController") {
     devType = "CONTROLLER"
  }  
  // yeah, I know there is no firmware compliance for switches. However the API show support for it.
  if (Tgroup.devtype == "CX") || (Tgroup.devtype == "ArubaSwitche") {
     devType = "HP"
  }  

  jsonPrep := fmt.Sprintf("{\"device_type\":\"%s\",\"group\":\"%s\",\"firmware_compliance_version\":\"%s\",\"reboot\":true,\"allow_unsupported_version\":true,\"compliance_scheduled_at\":0}",devType,gname,Tgroup.firmware)
...
func createGroup(central_info goCentral.Central_struct, gname string, Tgroup templateGroup) string {
  ...
  jsonPrep := ""
  if (Tgroup.devtype == "IAP") {
     jsonPrep = fmt.Sprintf("{\"group\":\"%s\", \"group_attributes\":{\"template_info\":{\"Wired\": %v, \"Wireless\": %v }, \"group_properties\":{\"AllowedDevTypes\": %s, \"Architecture\": \"%s\", \"ApNetworkRole\": \"%s\"}}}",gname,Tgroup.wired,Tgroup.wireless,Tgroup.AllowedDevTypes,Tgroup.Architecture,Tgroup.ApNetworkRole)
  }
  if (Tgroup.devtype == "MobilityController") {
	  jsonPrep = fmt.Sprintf("{\"group\":\"%s\", \"group_attributes\":{\"template_info\":{\"Wired\": %v, \"Wireless\": %v }, \"group_properties\":{\"AllowedDevTypes\": %s, \"Architecture\": \"%s\",\"GwNetworkRole\":\"%s\"}}}",gname,Tgroup.wired,Tgroup.wireless,Tgroup.AllowedDevTypes,TG.Architecture,Tgroup.GwNetworkRole)
  }
  if (Tgroup.devtype == "ArubaSwitche") || (Tgroup.devtype == "CX") {
     jsonPrep = fmt.Sprintf("{\"group\":\"%s\", \"group_attributes\":{\"template_info\":{\"Wired\": %v, \"Wireless\": %v }, \"group_properties\":{\"AllowedDevTypes\": %s, \"AllowedSwitchTypes\": %s, \"MonitorOnly\": [] }}}",gname,Tgroup.wired,Tgroup.wireless,Tgroup.AllowedDevTypes,Tgroup.AllowedSwitchTypes)
  }
