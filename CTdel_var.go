...
var appName = "CTdel_var"
var appVer = "1.5"
var appAuthor = "Michael Gresham"
var appAuthorEmail = "michael.gresham@hpe.com"
var pgmDescription = fmt.Sprintf("%s: Delete a variable in Central for a list of device serial numbers.",appName)
...
func del_variables(central_info goCentral.Central_struct, fname string) string {
  ...
  c := http.Client{Timeout: time.Duration(10) * time.Second}
  req, err := http.NewRequest("PUT", api_function_url, &data)
  if err != nil {
      fmt.Printf("error %s", err)
      return("ERROR")
  }
...
func p_check(central_info goCentral.Central_struct, fname string, oprefix string, variable string) {
...
//        fmt.Println(string(return_dict))
        del_dict, _, _, err := jsonparser.Get(jsonparser.Delete(return_dict, line,"variables",variable), line, "variables")
        if err != nil {
          fmt.Printf("error %s", err)
          return
        }
//        fmt.Println(string(del_dict))
         
        tmp_json := fmt.Sprintf("%s",string(del_dict))
//        fmt.Println(string(tmp_json))
...
func main() {
  ...
  variable := parser.String("","variable", &argparse.Options{Help: "Variable to delete"})
...
//    fmt.Printf("Central Refresh Token: %s\n",central_info.Refresh_token)
 ...
 if (*variable == "") {
   fmt.Print("\nProvide the Central variable to be deleted: ")
   fmt.Scanln(variable)
 }

  fmt.Println("Input file name:",*infile)
  fmt.Println("Variable to be deleted:",*variable)
...
//======================================================
  ...
  fmt.Println("--------------------------------")
  p_check(central_info,*infile,"pre-check",*variable)


//======================================================
// perform variable deletion 
//======================================================
  ...
  for key, element := range p_check_dict {
    ...
    if count == 250 {
      ...
      //create temp file
      tfile, err := ioutil.TempFile(os.TempDir(), "CTdel_var")
      if err != nil {
        log.Fatal(err)
      }
      ...
      if *test {
        ...
        response = del_variables(central_info, tfile.Name())
      }
...
//      defer os.Remove(tfile.Name())
  ...
  response := ""
  if *test {
    fmt.Println("SIMULATION del_variables")
    response = "NO ACTION"
  } else {
    response = del_variables(central_info, tfile.Name())
  }
...
//  fmt.Println("--------------------------------")
//  p_check(central_info,*infile,"post-check",*variable)
